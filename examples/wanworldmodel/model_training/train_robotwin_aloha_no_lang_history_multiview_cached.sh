...
# path
# =========================
cache_output_path="./outputs/WanWorldModel_film_w_history_multiview_cache"  # 第一阶段缓存输出目录。
...
train_dataset_type="unified"  # train 阶段读取第一阶段生成的缓存数据。
cache_dataset_repeat=1  # cache 阶段只遍历一次原始数据。
dataset_repeat=3  # train 阶段每个 epoch 内虚拟重复数据集的次数，用于增加训练步数。
dataset_num_workers=4  # 训练 DataLoader worker 数。
dataset_prefetch_factor=2  # 每个 worker 预取的 batch 数。
disable_dataset_persistent_workers=false  # 是否禁用 persistent DataLoader workers。
...
world_model_history_stride=8  # 每隔 n 帧取一段连续 4 帧 history；4 表示原来的连续 16 帧。
cache_world_model_history_dropout_prob=0.0  # cache 阶段不随机丢 history，保证缓存确定。
world_model_history_dropout_prob=0.3  # train 阶段从缓存训练时随机丢 history 的概率。

# =========================
# video size / sampling
# =========================
height=768  # multiview 总高度；三路 256 高视角在 latent 高度维堆叠。
...
# training
# =========================
cache_task="sft:data_process"  # cache 阶段只做数据预处理并写入缓存。
task="sft:train"  # train 阶段从缓存读取输入并训练。
...
find_unused_parameters=false  # DDP 是否查找未使用参数。
...
# eval
# =========================
cache_eval_steps=0  # cache 阶段禁用 eval。
cache_upload_video_steps=0  # cache 阶段禁用视频上传。
...
eval_dataset_num_workers=2  # eval DataLoader worker 数。
...
cache_save_training_checkpoint=false  # cache 阶段不保存 optimizer/scheduler/RNG/global step 等完整断点。
save_training_checkpoint=true  # train 阶段是否在 save_steps 时保存完整断点。
...
add_arg() {
  local -n args_ref="${current_args_name}"
  args_ref+=("$1" "$2")
}
...
add_common_args() {
  ...
  add_arg --dataset_prefetch_factor "${dataset_prefetch_factor}"
  add_flag --disable_dataset_persistent_workers "${disable_dataset_persistent_workers}"
  ...
  add_arg --world_model_history_stride "${world_model_history_stride}"
...
mkdir -p "${cache_output_path}" "${output_path}"
...
add_arg --dataset_type "${cache_dataset_type}"
add_arg --dataset_repeat "${cache_dataset_repeat}"
add_common_args

# World Model Dataset
add_arg --world_model_history_dropout_prob "${cache_world_model_history_dropout_prob}"

# Training
add_arg --task "${cache_task}"

# Eval
add_arg --eval_steps "${cache_eval_steps}"
add_arg --upload_video_steps "${cache_upload_video_steps}"

# Output / Checkpoints
add_arg --output_path "${cache_output_path}"
add_inverse_flag --disable_training_checkpoint "${cache_save_training_checkpoint}"
